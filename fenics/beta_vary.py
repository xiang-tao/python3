...
rho = 1800.0
num = 7
beta_arry = np.linspace(-0.03, 0.03, num)
wf = np.zeros(num)
wx = np.zeros(num)
wy = np.zeros(num)
...
cmpwww = np.array([beta_arry, wx, wy, wf])

np.savetxt('/home/xt/github/python3/fenics/cmpdata/beta_vary_data.txt', np.c_[cmpwww],
           fmt='%.16f', delimiter='\t')