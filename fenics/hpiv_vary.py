...
rho = 1800.0
num = 7
hpiv_arry = np.linspace(50, 110, num)
wf = np.zeros(num)
wx = np.zeros(num)
wy = np.zeros(num)
...
cmpwww = np.array([hpiv_arry, wx, wy, wf])

np.savetxt('/home/xt/github/python3/fenics/cmpdata/h_vary_data.txt', np.c_[cmpwww],
           fmt='%.16f', delimiter='\t')