...
def uh(ri):
    return r_in[ri]*r0*ww


def u0(ri, tj):
    return (r_in[ri]*r0+d*cos(theta_in[tj]))*wp
...
def f3(ri, tj):
    return (2*rho*hpiv**3*u0(ri, tj)**2/r0-rho*uh(ri)**2*hpiv**3/r0)/kk*dhhh_r(ri, tj)


def f4(ri, tj):
    return -2*rho*hpiv**3*omega0(tj)/r0*u0(ri,tj)/r_in[ri]/kk*dhhh_theta(ri, tj)
...
for i in range(1, r_in.size):
    for j in range(theta_in.size):
        ...
        Dn[k] = -c1(i, j) / hr ** 2
        D_n[k] = -c3(i, j) / hr ** 2
        b[k+1] = -f1(i, j) - f2(i, j)-f3(i,j)-f4(i,j)
        k += 1
