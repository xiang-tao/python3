...
"""
二维情形下的抛物方程求解，差分格式对应为Richardson(完全不稳定，因此画图得到的结果总是令人惊讶的)
由于该迭代方式一层的初始温度条件无法满足，需要两层，因此首先用Crank-Nicholson方式求取第二层的温度数据
"""
e = np.e
pi = np.pi
times = 5000
...
###################################################################################
# 下面开始借助已知的两层温度用Richardson格式计算剩下的温度层
u2 = np.zeros(N)
a2 = np.zeros((N, N))
a2 = matrix.richardson(a2, nx, ny, 2 * r1, 2 * r2)  # 调用richardson函数
data2 = np.zeros(5 * nx * ny - 2 * (nx + ny))  # 代表矩阵中非零元素的个数
indices2 = []
indptr2 = [0]
...
for i in range(nt):
    ...
    for m in range(nx):
        ...
        for n in range(ny):
            ...
            f2[k] = e ** (-t2) * np.sin(pi * x) * np.sin(pi * y) * (2 * pi ** 2 - 1)
            k += 1
    u2 = tools.csrAx(data2, indices2, indptr2, u1) - u0 + 2 * tau * f2
    u0 = u1
    u1 = u2
...
ax.set_title("Richardson Error t=0.1s hx=1/20 hy=1/20", fontproperties=zhfont1, fontsize=20)
